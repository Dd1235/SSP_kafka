...
The normal test suite is fast and does not require Kafka or Docker:

```bash
go test ./...
go test -race ./internal/...
go test ./internal/payload -fuzz=FuzzHeaderRoundTrip -fuzztime=10s
```

Latest local run on 2026-04-30:

| Check                                                                | Result                                                       |
| -------------------------------------------------------------------- | ------------------------------------------------------------ |
| `go test ./...`                                                      | PASS                                                         |
| `go test -race ./internal/...`                                       | PASS                                                         |
| `go test ./internal/payload -fuzz=FuzzHeaderRoundTrip -fuzztime=10s` | PASS, 2,777,680 fuzz executions                              |
| `go test -tags=integration ./... -run Integration -count=1`          | PASS; Kafka smoke test skipped unless `KAFKA_BROKERS` is set |

`cmd/bench/integration_test.go` is an opt-in end-to-end smoke test. It is
excluded from the default suite by the `integration` build tag, so it will not
run during `go test ./...`. When Kafka is available, run:

```bash
./run.sh start
KAFKA_BROKERS=localhost:9092 go test -tags=integration ./... -run Integration -count=1
```

That test creates a temporary topic and group, runs a small producer/consumer
benchmark, checks that messages and lag samples are recorded, and verifies the
JSON report can be written and decoded.
